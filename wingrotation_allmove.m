function[lattice]=wingrotation_allmove(wingno,geo,lattice,Raxle,hinge_pos,alpha,symmetry)
%%%
...
%    Spånga, 2021 08 05: Modded to cater for assymetric canard deflection
%   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    lemma1=((geo.nx+geo.fnx).*geo.ny);      %How many panels on SB side
    lemma2=(geo.symetric+1)';               %is wing symetric? 
    

    [a b]=size(lemma1);
    for i=1:b
        npan_m1(:,i)=lemma1(:,i).*lemma2;   %Total number of panels to work with
    end

    npan=sum(npan_m1,2);                    %Total number of panels to work with

    if symmetry
        alpha_v=alpha*ones(npan(wingno),1);     %deflection angle
    else
        alpha_v=alpha*[ones(npan(wingno)/2,1); -ones(npan(wingno)/2,1)]; %deflection angle)
    end
    
    if geo.symetric(wingno)
        %if symmetry
        %    hinge_pos_v=ones(npan(wingno),3).*hinge_pos;
        %else
            hinge_pos_v=[ones(npan(wingno)/2,3).*hinge_pos; ones(npan(wingno)/2,3).*(hinge_pos.*[1 -1 1])];
        %end
    end
    

    if wingno==1;                           %Which index to sart and stop with
        startindex=1;
        endindex=npan(1);
    else
    ...
    l.V(:,:,1)=lattice.VORTEX(startindex:endindex,:,1)-hinge_pos_v(:,1);
    l.V(:,:,2)=lattice.VORTEX(startindex:endindex,:,2)-hinge_pos_v(:,2);
    l.V(:,:,3)=lattice.VORTEX(startindex:endindex,:,3)-hinge_pos_v(:,3);

    l.C(:,1)=lattice.COLLOC(startindex:endindex,1)-hinge_pos_v(:,1);
    l.C(:,2)=lattice.COLLOC(startindex:endindex,2)-hinge_pos_v(:,2);
    l.C(:,3)=lattice.COLLOC(startindex:endindex,3)-hinge_pos_v(:,3);

    l.XYZ(:,:,1)=lattice.XYZ(startindex:endindex,:,1)-hinge_pos_v(:,1);
    l.XYZ(:,:,2)=lattice.XYZ(startindex:endindex,:,2)-hinge_pos_v(:,2);
    l.XYZ(:,:,3)=lattice.XYZ(startindex:endindex,:,3)-hinge_pos_v(:,3);
    ...
    l2.C=trot4(Raxle,l.C,alpha_v)';   %Rotating collocationpoints
    l2.N=trot4(Raxle,l.N,alpha_v)';   %Rotating Normals    
    ...
    l2.V(:,:,1)=l2.V(:,:,1)+hinge_pos_v(:,1);
    l2.V(:,:,2)=l2.V(:,:,2)+hinge_pos_v(:,2);
    l2.V(:,:,3)=l2.V(:,:,3)+hinge_pos_v(:,3);

    l2.C(:,1)=l2.C(:,1)+hinge_pos_v(:,1);
    l2.C(:,2)=l2.C(:,2)+hinge_pos_v(:,2);
    l2.C(:,3)=l2.C(:,3)+hinge_pos_v(:,3);

    l2.XYZ(:,:,1)=l2.XYZ(:,:,1)+hinge_pos_v(:,1);
    l2.XYZ(:,:,2)=l2.XYZ(:,:,2)+hinge_pos_v(:,2);
    l2.XYZ(:,:,3)=l2.XYZ(:,:,3)+hinge_pos_v(:,3);
